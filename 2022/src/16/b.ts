...
while (queue.length) {
  const { valveYou, valveElephant, timeRemaining, openedValves, flowScore } =
    queue.shift()!;

  for (const yourRelevantValve of relevantValves) {
    for (const elepRelevantValve of relevantValves) {
      if (yourRelevantValve === elepRelevantValve) continue;

      const distanceToYourValve =
        distanceMap.get(`${valveElephant.name}-${yourRelevantValve.name}`) ?? 0;
      const distanceToElepValve =
        distanceMap.get(`${valveYou.name}-${elepRelevantValve.name}`) ?? 0;

      if (
        distanceToYourValve <= timeRemaining &&
        yourRelevantValve !== valveYou
      ) {
      }
    }
  }
  ...
  for (const relevantValve of relevantValves) {
    ...
    const distanceToValve =
      distanceMap.get(`${valveYou.name}-${relevantValve.name}`) ?? 0;
